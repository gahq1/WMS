	...
	<form action="<%=path %>/shoufei!add.action" name="formAdd" method="post">
		<table class="table table-hover  table-bordered table-striped">
			...
			<tr>
				...
				<td width="50%">
					<select id="userno" name="userno" onchange="findByUserid()">
						<c:forEach items="${userList}" var="User">
							<option value="${User.id}" >
									${User.id}_${User.loginname}_${User.realname}
							</option>
						</c:forEach>
					</select>
				</td>
			...
			<tr>
				...
				<td width="50%">
					<select id="shuibiaono" name="shuibiaono">
						<c:forEach items="${listuserid}" var="Shuibiao">
							<option value="${Shuibiao.biaono}">
<%--								<c:if test="${Shuibiao.userId == User.id}"></c:if>--%>
									${Shuibiao.biaono}
							</option>
